...
class ProcessingTableResearchScreen(
	...
	override fun init() {
		...
		researchButton = ButtonWidget
			.builder(Text.translatable("container.${ProjectGlitch.MOD_ID}.processing_table_research.research_button")) {
				selectedAdvancement?.also { selectedAdvancement ->
					ClientPlayNetworking.send(
						ProjectGlitch.PERFORM_RESEARCH_PACKET_ID,
	...
	override fun render(context: DrawContext, mouseX: Int, mouseY: Int, delta: Float) {
		...
		if (selectedTab?.root?.advancementEntry?.id != ProjectGlitch.ResearchAdvancement.ROOT_ID) {
			advancementsScreen.selectTab(advancementHandler.get(ProjectGlitch.ResearchAdvancement.ROOT_ID))
		}
	...
	companion object {
		private val TEXTURE = ProjectGlitch.identifier("textures/gui/container/processing_table_research.png")
	}
}
