...
class ProcessingTableBlock(
	...
	override fun <T : BlockEntity> getTicker(world: World, state: BlockState, type: BlockEntityType<T>): BlockEntityTicker<T>? {
		...
		if (type != ProjectGlitch.PROCESSING_TABLE_BLOCK_ENTITY) return null
	...
	private fun createBlockEntityItemTag(world: WorldView, pos: BlockPos, state: BlockState): NbtCompound? {
		...
		BlockEntity.writeIdToNbt(blockEntityTag, ProjectGlitch.PROCESSING_TABLE_BLOCK_ENTITY)
	...
	override fun onBreak(world: World, pos: BlockPos, state: BlockState, player: PlayerEntity): BlockState {
		if (!world.isClient) {
			...
			val itemStack = ProjectGlitch.PROCESSING_TABLE_BLOCK_ITEM.defaultStack
	...
	override fun onUse(state: BlockState, world: World, pos: BlockPos, player: PlayerEntity, hand: Hand, hit: BlockHitResult): ActionResult {
		...
		when (val blockEntity = world.getBlockEntity(pos)) {
			...
			is ProcessingTableResearchBlockEntity -> {
				...
				(player as ServerPlayerEntity).grantAdvancement(ProjectGlitch.ResearchAdvancement.ROOT_ID)
