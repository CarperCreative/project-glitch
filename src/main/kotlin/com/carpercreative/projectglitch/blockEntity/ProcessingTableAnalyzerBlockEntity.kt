package com.carpercreative.projectglitch.blockEntity
...
class ProcessingTableAnalyzerBlockEntity(
	...
	ProjectGlitch.PROCESSING_TABLE_BLOCK_ENTITY,
	pos,
	state,
), SidedInventory {
	...
	override fun getContainerName(): Text {
		return Text.translatable("container.${ProjectGlitch.MOD_ID}.processing_table_analyzer")
	}
	...
	override fun readNbt(nbt: NbtCompound) {
		val subNbt = nbt.getCompound(ProjectGlitch.PROCESSING_TABLE_ANALYZER_ID.toString())
	...
	override fun writeNbt(nbt: NbtCompound) {
		...
		nbt.put(ProjectGlitch.PROCESSING_TABLE_ANALYZER_ID.toString(), subNbt)
	}
	...
	object Ticker : BlockEntityTicker<ProcessingTableAnalyzerBlockEntity> {
		override fun tick(world: World, pos: BlockPos, state: BlockState, blockEntity: ProcessingTableAnalyzerBlockEntity) {
			...
			if (blockEntity.analysisTime >= ANALYSIS_DURATION) {
				// Analysis finished.
				if (analyzedStack.isOf(ProjectGlitch.PROBE_ITEM)) {
		...
		private fun analyzeProbe(blockEntity: ProcessingTableAnalyzerBlockEntity, stack: ItemStack) {
			if (!stack.isOf(ProjectGlitch.PROBE_ITEM)) return
			...
			if (cancerBlob == null) {
				prependBook(blockEntity, Text.translatable("${ProjectGlitch.MOD_ID}.analysis.invalid"))
				return
			}
			...
			prependBook(
				blockEntity,
				Text.translatable(
					"${ProjectGlitch.MOD_ID}.analysis.results",
					...
					Text.empty().also { text ->
						cancerBlob.treatments.forEach { treatment ->
							text.append(
								Text.translatable(
									"${ProjectGlitch.MOD_ID}.analysis.treatment_item",
					...
					Text.empty().also { notes ->
						if (cancerBlob.isMetastatic) {
							notes.append(Text.translatable("${ProjectGlitch.MOD_ID}.analysis.is_metastatic"))
						}
					},
				),
			)
	...
	companion object {
		...
		fun getAnalysisOutput(analyzedStack: ItemStack): ItemStack {
			return when {
				analyzedStack.isOf(ProjectGlitch.PROBE_ITEM) -> {
					ProbeItem.setSampleCancerBlobId(analyzedStack, null)
					analyzedStack
				}
				analyzedStack.isOf(ProjectGlitch.GLITCH_MATERIAL_ITEM) -> {
					analyzedStack.decrement(1)
					ProjectGlitch.RESEARCH_ITEM.defaultStack
				}
		...
		fun analysisRequiresBook(itemStack: ItemStack): Boolean {
			return itemStack.isOf(ProjectGlitch.PROBE_ITEM)
		}

		private val PROCESSING_DONE_SOUND = SoundEvent.of(ProjectGlitch.identifier("block.processing_table.processing_done"), 32f)
	}
}
